...
OptInfo = namedtuple("OptInfo",
    ...
    "q1", "q2", "piMu", "piLogStd", "qMeanDiff", "alpha", "lambda_"])
...
class SAC(RlAlgorithm):
    ...
    def optimize_agent(self, itr, samples=None, sampler_itr=None):
        ...
        for _ in range(self.updates_per_optimize):
            ...
            losses, values = self.loss(samples_from_replay, itr)
            ...
            q_grad_norm = torch.nn.utils.clip_grad_norm_(self.agent.q_parameters(),
                self.clip_grad_norm)
            self.q_optimizer.step()

            grad_norms = (q_grad_norm, pi_grad_norm)

            self.append_opt_info_(itr, opt_info, losses, grad_norms, values)
    ...
    def _lambda(self, itr):
        """
        y = ax + b
        y(0) = b = self.lambda_
        y(end) = 0 = a*end + self.lambda_
        a = -self.lambda_ / end
        y = self.lambda_ - itr * self.lambda_ / end
        = self.lambda_ * (1 - itr / end)
        """
        return max(0, self.lambda_ * (1 - itr / int(5e4)))
    ...
    def append_opt_info_(self, itr, opt_info, losses, grad_norms, values):
        """In-place."""
        q1_loss, q2_loss, pi_loss, alpha_loss = losses
        ...
        opt_info.q1GradNorm.append(torch.tensor(q_grad_norm).item())  # backwards compatible
        ...
        opt_info.lambda_.append(self._lambda(itr))
