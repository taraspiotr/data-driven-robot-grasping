...
class SquashedGaussianActor(nn.Module):
    def __init__(self, obs_dim, act_dim, hidden_sizes, activation, act_limit):
        super().__init__()
        self.net = self.init_net(obs_dim, act_dim, hidden_sizes, activation)
...
class SquashedGaussianMLPActor(SquashedGaussianActor):
    def init_net(self, obs_dim, act_dim, hidden_sizes, activation):
        return mlp([obs_dim] + list(hidden_sizes), activation, activation)
...
class SquashedGaussianCNNActor(SquashedGaussianActor):
    def init_net(self, obs_dim, act_dim, hidden_sizes, activation):
        ...
        return nn.Sequential(OrderedDict([("encoder", encoder), ("head", head),]))


class QFunction(nn.Module):
    def __init__(self, obs_dim, act_dim, hidden_sizes, activation):
        super().__init__()
        self.q = self.init_net(obs_dim, act_dim, hidden_sizes, activation)
...
class MLPQFunction(QFunction):
    def init_net(self, obs_dim, act_dim, hidden_sizes, activation):
        return mlp([obs_dim + act_dim] + list(hidden_sizes) + [1], activation)
...
class MLPActorCritic(nn.Module):
    def __init__(
        ...
        obs_dim = 4  # np.prod(observation_space.shape)
...
class CNNActorCritic(nn.Module):
    def __init__(
        ...
        # build policy and value functions
        self.pi = SquashedGaussianCNNActor(
            obs_dim, act_dim, hidden_sizes, activation, act_limit
        )
        self.q1 = CNNQFunction(obs_dim, act_dim, hidden_sizes, activation)
        self.q2 = CNNQFunction(obs_dim, act_dim, hidden_sizes, activation)

        self.q1.q.encoder.copy_conv_weights_from(self.pi.net.encoder)
        self.q2.q.encoder.copy_conv_weights_from(self.pi.net.encoder)
